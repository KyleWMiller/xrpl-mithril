...
pub enum TxError {
    ...
    Client(#[from] xrpl_mithril_client::ClientError),

    /// Error from the wallet/signing layer.
    #[error("wallet error: {0}")]
    Wallet(#[from] xrpl_mithril_wallet::WalletError),

    /// Error from the binary codec layer.
    #[error("codec error: {0}")]
    Codec(#[from] xrpl_mithril_codec::error::CodecError),

    /// Error from the type layer.
    #[error("type error: {0}")]
    Type(#[from] xrpl_mithril_types::TypeError),
