...
use xrpl_mithril_models::transactions::wrapper::{
    Signable, TypedSignedTransaction, UnsignedTransaction,
};
use xrpl_mithril_models::transactions::Transaction;
...
/// signer in `xrpl-wallet`. The existing `xrpl_mithril_wallet::sign()` function is
/// called internally.
///
/// # Examples
///
...
pub fn sign_transaction<T: Signable>(
    ...
    let signed = xrpl_mithril_wallet::sign(&map, wallet)?;
